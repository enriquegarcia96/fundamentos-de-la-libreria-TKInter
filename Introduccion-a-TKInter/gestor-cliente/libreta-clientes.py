...
# funcion que se encarga de renderizar los datos, para que se miren reflejados
def render_clientes():
    rows = c.execute('SELECT * FROM cliente').fetchall()

    tree.delete(*tree.get_children())  # elimina todos los hijos
...
def nuevo_cliente():
    def guardar():
        if not nombre.get():
            ...
            return
        if not telefono.get():
            messagebox.showerror('Erro', 'El telefono es obligatorio')
            return
        if not empresa.get():
            messagebox.showerror('Erro', 'El campo empresa es obligatorio')
            return
        ...
        insertar(cliente)
        top.destroy()  # destruye la ventana de nuevo cliente.
